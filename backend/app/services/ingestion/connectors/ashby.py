...
class AshbyConnector(BaseConnector):
    def fetch(self, source_config: Dict[str, Any]) -> ConnectorResultV1:
        ...
        source_name = source_config.get("name", f"Ashby:{board}")
        if not board:
        ...
        logger.info(f"AshbyConnector: fetching jobs for board '{board}'...")
        data = self._http_get_with_retry(url)
        duration = time.time() - start_time

        if data and "jobs" in data:
            jobs = data["jobs"]
            logger.info(f"AshbyConnector: fetched {len(jobs)} jobs for '{board}' in {duration:.2f}s")
            return ConnectorResultV1(
                source=source_name,
                duration=duration,
            ...
            logger.warning(f"AshbyConnector: failed to fetch jobs for '{board}'")
            return ConnectorResultV1(
                source=source_name,
                duration=duration,
                jobs_fetched=0,
